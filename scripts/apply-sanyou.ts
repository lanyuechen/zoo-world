/**
 * 按《有重要生态、科学、社会价值的陆生野生动物名录》（2023）写入三有标签（物种详情 JSON）
 */
...
function main() {
  ...
  try {
    forEachSkeleton((rec) => {
      if (rec.kingdom?.latin !== 'Animalia') return
      animal += 1

      const existing = loadDetail(resolveJsonPath(rec))
      const { sanyou, tags } = applySanyouTags(
        { ...rec, tags: existing?.tags as string[] | undefined },
        lookup,
      )
      ...
      if (patchDetail(rec, { sanyou, tags }, { createIfMissing: !!sanyou })) written += 1
    })
  } catch (e) {
  ...
  updateMeta((meta) => {
    ...
    const note =
      '动物「三有」标签依据《有重要生态、科学、社会价值的陆生野生动物名录》（2023）写入物种详情'
    if (!notes.includes(note)) notes.push(note)
    meta.notes = notes
  })

  console.log('《三有名录》匹配完成（写入物种详情 JSON）')
