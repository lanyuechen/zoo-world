/**
 * 按《国家重点保护野生动物名录》（2021）为动物界物种写入 status（物种详情 JSON）
 *
...
function main() {
  ...
  try {
    forEachSkeleton((rec) => {
      ...
      if (status) matched += 1
      if (patchDetail(rec, { status }, { createIfMissing: !!status })) written += 1
    })
  } catch (e) {
  ...
  updateMeta((meta) => {
    ...
    const note =
      '动物保护等级依据《国家重点保护野生动物名录》（2021）匹配写入物种详情 status'
    if (!notes.includes(note)) notes.push(note)
    meta.notes = notes
  })

  console.log('《国家重点保护野生动物名录》匹配完成（写入物种详情 JSON）')
  console.log(`  名录物种条目: ${list.species.length}（另有阶元规则 ${list.taxonRules.length}）`)
  console.log(`  库内动物界: ${animal}`)
  console.log(`  已匹配保护等级: ${matched}`)
